...
health_per_inhabitantgraphics <- read_excel("Data/base-ic-evol-struct-pop-2013.xlsx", skip = 5)
health_per_inhabitantgraphics2 <- read_excel("Data/base-ic-evol-struct-pop-2013.xlsx", skip = 4)

names <- top_n(health_per_inhabitantgraphics2, 1)
colnames(equip_dep)[colnames(equip_dep) == "dep"] <- "DEP"
equip_dep <- 
  equip_dep %>% 
  filter(as.numeric(DEP) < 97)

health_per_inhabitantgraphics_by_dep <- health_per_inhabitantgraphics %>% 
  group_by(DEP) %>% 
...
maternities$nb_2012 <- 
  maternities$nb_2012 / (health_per_inhabitantgraphics_by_dep$C13_POP15P_CS4_sum_pop + health_per_inhabitantgraphics_by_dep$C13_POP15P_CS5_sum_pop)

maternities$nb_2017 <- 
  maternities$nb_2017 / (health_per_inhabitantgraphics_by_dep$C13_POP15P_CS4_sum_pop + health_per_inhabitantgraphics_by_dep$C13_POP15P_CS5_sum_pop)
...
total <- merge(equip_dep, health_per_inhabitantgraphics_by_dep, by = "DEP")
total %>% distinct(DEP)
...
urg <- merge(urgences, health_per_inhabitantgraphics_by_dep[,1:2], by = "DEP")
urg$2012 <- 
